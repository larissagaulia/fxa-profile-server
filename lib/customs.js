...
Customs.prototype.checkAuthenticated = function (action, ip, token, uid) {
  logger.info('customs.checkAuthenticated', { ip: ip, token: token, uid: uid});
  ...
  .then(
    ...
    function (err) {
      logger.error('customs.checkAuthenticated', {ip: ip, token: token, uid: uid, err: err });
